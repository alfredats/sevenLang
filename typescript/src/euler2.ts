...
export function sumEvenFibNumUpTo (n: number): number {
    return allEvenFibNumUpTo(n).reduce((acc,currVal) => acc + currVal)
}


export function* generateFibonacci(prev: number,next: number): Generator<number> {
    let temp = 0
    while (true) {
        yield (prev + next)
        temp = prev
...
function allEvenFibNumUpTo (n: number): number[] {
    const arr: number[] = []
