 ...
 *
 * Height of a node: max( left height, right height) + 1
 * Height of null: -1
 *
 * balance factor of a node: left height - right height
 * - a node is balanced if balance factor less than |1| 
 * 
 * rotations occur when node is unbalanced
 *
 * - right rotation occurs if root and left child are left heavy
 * - left-right rotation occurs if root is left heavy, left child is right heavy
 *
 * - left rotation occurs if root and right child are right heavy
 * - right left rotation occurs if root is right heavy, right child is left heavy
 */
public class AVL<T extends Comparable<? super T>> {
  ...
  public void add(T data) {
    ...
    this.root = rAdd(this.getRoot(), data);
  }
  ...
  public T remove(T data) {
    ...
    AVLNode<T> foo = new AVLNode<T>(null);
    this.root = rRemove(this.getRoot(), data, foo);
    this.size--;
    return foo.getData();
  }

  private AVLNode<T> rRemove(AVLNode<T> root, T data, AVLNode<T> tmp) {
    if (root == null) {
      throw new NoSuchElementException();
    }

    if (root.getData().compareTo(data) > 0) {
      root.setLeft(rRemove(root.getLeft(), data, tmp));
    } else if (root.getData().compareTo(data) < 0) {
      root.setRight(rRemove(root.getRight(), data, tmp));
    } else {
      ...
      if (root.getLeft() == null) {
        ...
        // has both children
          ...
          AVLNode<T> bar = new AVLNode<>(null);
          root.setRight(this.removeSucc(root.getRight(), bar));
          root.setData(bar.getData());
        }
      }
    }
    root = this.balance(root);
  ...
  private AVLNode<T> removeSucc(AVLNode<T> root, AVLNode<T> dummy) {
    if (null == root.getLeft()) {
      dummy.setData(root.getData());
      return root.getRight();
    }
    root.setLeft(this.removeSucc(root.getLeft(), dummy));
    root = this.balance(root);
    return root;
  }
  ...
  private AVLNode<T> rotateLeft(AVLNode<T> currentNode) {
    ...
    AVLNode<T> nuRoot = currentNode.getRight();
    currentNode.setRight(nuRoot.getLeft());
    nuRoot.setLeft(currentNode);
    this.updateHeightAndBF(currentNode);
    this.updateHeightAndBF(nuRoot);
    return nuRoot;
  }
  ...
  private AVLNode<T> rotateRight(AVLNode<T> currentNode) {
    ...
    AVLNode<T> nuRoot = currentNode.getLeft();
    currentNode.setLeft(nuRoot.getRight());
    nuRoot.setRight(currentNode);
    this.updateHeightAndBF(currentNode);
    this.updateHeightAndBF(nuRoot);
    return nuRoot;
  }
